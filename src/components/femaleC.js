...
const Model_Female = ({ heightScale, showBones }) => {
  const meshRef = useRef();
  const gltf = useLoader(GLTFLoader, 'models/human/female/female.gltf');
        ...
        if (object.isMesh && object.skeleton) {
          ...
          bones.forEach((bone) => {
            ...
            if (faceBones.includes(bone.name)) {
              ...
              bone.scale.x = bone.scale.z = 1 + (heightScale - 1) * 0;
              ...
              bone.scale.y = 1 + (heightScale - 1) * -1.2;
...
const FemaleC = () => {
    ...
    const changeHeight = useCallback((change) => {
      setHeight((prevHeight) => {
        ...
        return newHeight >= 140 && newHeight <= 230 ? newHeight : prevHeight;
      });
    }, []);
    ...
    return (
      <div style={{ position: 'relative', width: '100vw', height: '100vh', margin: 0, padding: 0 }}>
        ...
        <Model_Female heightScale={heightScale} showBones={showBones} />
